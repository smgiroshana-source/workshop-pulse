...
export function WorkshopProvider({ children }) {
  ...
  // Escape user-entered text before it goes into document.write HTML — a part
  // named "100<50mm bolt" (or worse, a crafted name) must never become markup
  ...
  const openPDF = (title, bodyHtml) => {
    try {
      ...
      w.document.write("<!DOCTYPE html><html><head><title>" + esc(title) + "</title><style>" + pdfStyles + "</style></head><body><button class=\"print-btn no-print\" onclick=\"window.print()\">Print / Save PDF</button>" + bodyHtml + "</body></html>")
      w.document.close()
    } catch (e) {
  ...
  const generateEstimatePDF = (est) => {
    ...
    cats.forEach(cat => {
      ...
      catEntries.forEach((e, i) => {
        ...
        tableRows += "<tr><td>" + (i + 1) + "</td><td>" + esc(name) + " " + remarkTag + "</td><td class=\"text-center\">" + e.qty + "</td><td class=\"text-right mono\">Rs." + e.rate.toLocaleString() + "</td><td class=\"text-right mono bold\">Rs." + (e.qty * e.rate).toLocaleString() + "</td></tr>"
      })
    })
    if (est.sundries && est.sundries.length) {
      ...
      est.sundries.forEach((s, i) => {
        tableRows += "<tr><td>" + (i + 1) + "</td><td>" + esc(s.name) + (s.remarks === "M/R" ? " <span class=\"tag tag-mr\">M/R</span>" : "") + "</td><td class=\"text-center\">" + (s.qty || 1) + "</td><td class=\"text-right mono\">Rs." + (s.rate || 0).toLocaleString() + "</td><td class=\"text-right mono bold\">Rs." + ((s.rate || 0) * (s.qty || 1)).toLocaleString() + "</td></tr>"
      })
    }
  ...
  const generateInvoicePDF = (inv) => {
    ...
    ci.forEach((item, i) => {
      rows += "<tr><td>" + (i + 1) + "</td><td>" + esc(item.description) + (item.remarks ? " <span class=\"tag " + (item.remarks === "S/H" ? "tag-sh" : item.remarks === "M/R" ? "tag-mr" : "tag-us") + "\">" + item.remarks + "</span>" : "") + "</td><td class=\"text-center\">" + item.category + "</td><td class=\"text-center\">" + item.qty + "</td><td class=\"text-right mono\">Rs." + item.unit_price.toLocaleString() + "</td><td class=\"text-right mono bold\">Rs." + (item.unit_price * item.qty).toLocaleString() + "</td></tr>"
    })
    // The customer/settlement discount is a PRIVATE arrangement between the
    // workshop and the customer. It must NEVER appear on (or reduce) an
    // insurance invoice — printing it both disclosed private pricing and made
    // the insurer pay less, so the workshop ate the discount twice. Insurance
    // documents show the gross figure; direct-job invoices still show it.
    const docTotal = isInsurance ? afterInsDisc : afterAllDisc
    ...
    if (custDisc > 0 && !isInsurance) totalSection += "<tr><td colspan=\"5\" class=\"text-right\" style=\"color:#e53935\">Customer Discount</td><td class=\"text-right mono\" style=\"color:#e53935\">-Rs." + (afterInsDisc - afterAllDisc).toLocaleString() + "</td></tr>"
    totalSection += "<tr class=\"total-row\"><td colspan=\"5\" class=\"text-right\" style=\"font-size:16px\">NET TOTAL</td><td class=\"text-right mono\" style=\"font-size:16px\">Rs." + docTotal.toLocaleString() + "</td></tr>"
    ...
    if (isInsurance && (excess > 0 || insShare > 0)) {
      ...
      totalSection += "<tr><td colspan=\"5\" class=\"text-right\" style=\"color:#e65100\">Customer Portion" + (excess > 0 ? " (Excess)" : "") + "</td><td class=\"text-right mono\" style=\"color:#e65100\">Rs." + Math.max(0, docTotal - insShare).toLocaleString() + "</td></tr>"
    }
    const payRows = (inv.payments || []).map((p, i) => "<tr><td>" + new Date(p.date).toLocaleDateString() + "</td><td>" + (p.type === "insurance" ? "Insurance" : p.method) + "</td><td>" + esc(p.reference || "-") + "</td><td class=\"text-right mono\">Rs." + p.amount.toLocaleString() + "</td></tr>").join("")
    const html = "<div class=\"header\"><div><div class=\"shop-name\">" + SHOP.name + "</div><div class=\"shop-detail\">" + SHOP.addr + "</div><div class=\"shop-detail\">" + SHOP.phone + "</div></div><div><div class=\"doc-title\">INVOICE</div><div class=\"doc-sub\">" + inv.invoice_number + "</div><div class=\"doc-sub\">" + new Date(inv.created_at).toLocaleDateString() + "</div></div></div><div class=\"info-grid\"><div><div class=\"info-label\">Vehicle</div><div class=\"info-value\">" + esc(jobInfo.vehicle_reg) + "</div></div><div><div class=\"info-label\">Make / Model</div><div class=\"info-value\">" + esc(jobInfo.vehicle_make) + " " + esc(jobInfo.vehicle_model) + "</div></div><div><div class=\"info-label\">Customer</div><div class=\"info-value\">" + esc(jobInfo.customer_name) + "</div></div><div><div class=\"info-label\">" + (isInsurance ? "Insurance" : "Phone") + "</div><div class=\"info-value\">" + esc(isInsurance ? jobInfo.insurance_name : jobInfo.customer_phone) + "</div></div></div><table><thead><tr><th>#</th><th>Description</th><th class=\"text-center\">Cat</th><th class=\"text-center\">Qty</th><th class=\"text-right\">Rate</th><th class=\"text-right\">Amount</th></tr></thead><tbody>" + rows + totalSection + "</tbody></table>" + (payRows ? "<h3 style=\"margin:18px 0 8px;font-size:15px\">Payments Received</h3><table><thead><tr><th>Date</th><th>Type</th><th>Ref</th><th class=\"text-right\">Amount</th></tr></thead><tbody>" + payRows + "</tbody></table>" : "") + "<div style=\"margin:18px 0\"><div><strong>Total:</strong> Rs." + docTotal.toLocaleString() + "</div><div><strong>Paid:</strong> Rs." + paid.toLocaleString() + "</div><div><strong>Balance:</strong> <span style=\"color:" + (docTotal - paid > 0 ? "#e53935" : "#2e7d32") + ";font-weight:700\">Rs." + (docTotal - paid).toLocaleString() + "</span></div></div><div class=\"stamp\"><div class=\"stamp-box\"><div class=\"stamp-line\">" + SHOP.name + "</div></div><div class=\"stamp-box\"><div class=\"stamp-line\">Customer Signature</div></div></div><div class=\"footer\"><span>" + SHOP.name + "</span><span>Generated " + new Date().toLocaleDateString() + "</span></div>"
  ...
  const generatePQPDF = () => {
    ...
    partsQuotation.forEach((p, i) => {
      ...
      rows += "<tr><td>" + (i + 1) + "</td><td>" + esc(p.partName) + "</td><td class=\"text-center\"><span class=\"tag " + (p.remarks === "S/H" ? "tag-sh" : p.remarks === "M/R" ? "tag-mr" : "tag-us") + "\">" + p.remarks + "</span></td><td>" + esc(p.supplier || "--") + "</td><td class=\"text-right mono bold\">" + appr + "</td></tr>"
    })
    const pqTotalPrice = partsQuotation.reduce((s, p) => s + (p.quotedPrice || 0), 0)
    ...
    const html = "<div class=\"header\"><div><div class=\"shop-name\">" + SHOP.name + "</div><div class=\"shop-detail\">" + SHOP.addr + "</div><div class=\"shop-detail\">" + SHOP.phone + "</div></div><div><div class=\"doc-title\">PARTS QUOTATION</div><div class=\"doc-sub\">" + (activeJob ? activeJob.jobNumber : "") + " - " + esc(jobInfo.insurance_name) + "</div><div class=\"doc-sub\">" + new Date().toLocaleDateString() + "</div></div></div><div class=\"info-grid\"><div><div class=\"info-label\">Vehicle</div><div class=\"info-value\">" + jobInfo.vehicle_reg + "</div></div><div><div class=\"info-label\">Make / Model</div><div class=\"info-value\">" + jobInfo.vehicle_make + " " + jobInfo.vehicle_model + "</div></div><div><div class=\"info-label\">Customer</div><div class=\"info-value\">" + jobInfo.customer_name + "</div></div><div><div class=\"info-label\">Insurance</div><div class=\"info-value\">" + esc(jobInfo.insurance_name) + "</div></div></div><table><thead><tr><th>#</th><th>Part Description</th><th class=\"text-center\">Type</th><th>Supplier</th><th class=\"text-right\">Approved</th></tr></thead><tbody>" + rows + "</tbody></table>" + (pqLumpMode && pqLumpSum ? "<div class=\"summary-box\" style=\"text-align:center\"><div style=\"font-size:13px;color:#666\">Insurance Approved (Lump Sum)</div><div style=\"font-size:24px;font-weight:700;font-family:monospace;margin-top:4px\">Rs." + pqLumpSum.toLocaleString() + "</div></div>" : "") + "<div class=\"stamp\"><div class=\"stamp-box\"><div class=\"stamp-line\">" + SHOP.name + "</div></div><div class=\"stamp-box\"><div class=\"stamp-line\">Insurance Assessor</div></div></div><div class=\"footer\"><span>" + SHOP.name + "</span><span>Generated " + new Date().toLocaleDateString() + "</span></div>"
  ...
  // ═══ STAGE TRANSITIONS ═══
  const advanceStage = (toStage, opts = {}) => {
    // Approval gate: an insurance job cannot leave est_ready without an
    // approved estimate — previously the pipeline button skipped approval
    // entirely and the job dead-ended at invoicing. finalizeApproval passes
    // fromApproval because its setEstimates hasn't committed yet in this tick.
    if (!opts.fromApproval && jobStage === "est_ready" && toStage !== "est_ready" && isInsurance && !estimates.some(e => e.status === "approved")) {
      tt("⚠️ No approved estimate yet — record the insurance approval first")
      return
    }
    setJobStage(toStage)
  ...
  const finalizeApproval = () => {
    ...
    if (jobStage === "est_ready" && isInsurance) advanceStage("approved_dismantle", { fromApproval: true })
    ...
    if (jobStage === "est_ready" && isDirectJob) {
      ...
      if (nextInPipeline) advanceStage(nextInPipeline, { fromApproval: true })
    }
    setScreen("job")
  }
