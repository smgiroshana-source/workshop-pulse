...
import { C, FONT, MONO, inp, btn, btnSm, card, NavBar, fmt, genId, Icon } from "../WorkshopContext"
    ...
    <>
      ...
      <div style={{ ...card, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 16 }}>
          <thead><tr style={{ borderBottom: `2px solid ${C.border}` }}><th style={{ textAlign: "left", padding: "10px 6px", color: C.sub, fontSize: 14 }}>Part</th>{jobCats.map(c => <th key={c.key} style={{ textAlign: "right", padding: "10px 6px", color: c.color, fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.3 }}>{c.short}</th>)}</tr></thead>
          <tbody>
      ...
      <div style={{ ...card, padding: "14px 16px" }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: C.sub, marginBottom: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>Other Items / Sundries</div>
        ...
        {sundryItems.map(si => <div key={si.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
          <input value={si.name} onChange={e => setSundryItems(p => p.map(x => x.id === si.id ? { ...x, name: e.target.value } : x))} onBlur={e => { const trimmed = e.target.value.trim(); setSundryItems(p => p.map(x => x.id === si.id ? { ...x, name: trimmed } : x)) }} style={{ flex: 1, padding: "8px 10px", background: C.bg, border: "none", borderRadius: 8, color: C.text, fontSize: 16, fontFamily: FONT, outline: "none" }} />
          {si.remarks === "M/R" ? <span onClick={() => setSundryItems(p => p.map(x => x.id === si.id ? { ...x, remarks: "", rate: 0 } : x))} style={{ fontFamily: MONO, fontSize: 16, fontWeight: 700, color: C.accent, background: C.accent + "12", padding: "4px 12px", borderRadius: 8, cursor: "pointer" }}>M/R</span>
            : <><input type="number" min="0" value={si.rate || ""} onChange={e => { const v = Number(e.target.value); if (v < 0) return; setSundryItems(p => p.map(x => x.id === si.id ? { ...x, rate: v || 0 } : x)) }} placeholder="0" style={{ width: 80, padding: "8px 10px", background: C.bg, border: "none", borderRadius: 8, color: C.text, fontSize: 16, fontFamily: MONO, fontWeight: 700, textAlign: "right", outline: "none" }} />
      ...
      {/* Keep selEst and current parts/entries — clearing them here used to wipe the estimate being edited */}
      <button onClick={() => setScreen("est_parts")} style={{ ...btn(C.bg, C.accent), marginBottom: 8 }}>Edit Parts List</button>
