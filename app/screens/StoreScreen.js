...
function PODetail({ po, onBack, onUpdate, onCreateGRN, onDelete, grns, tt }) {
  ...
  // Print — user-entered strings escaped, popup-block handled
  const esc = (v) => String(v ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;")
  const printPO = () => {
    const itemRows = items.map((i, idx) => `<tr><td style="padding:8px;border-bottom:1px solid #eee">${idx + 1}</td><td style="padding:8px;border-bottom:1px solid #eee">${esc(i.name)}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:center">${i.qty} ${esc(i.unit)}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">Rs.${(i.unitPrice || 0).toLocaleString()}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right;font-weight:700">Rs.${(i.qty * (i.unitPrice || 0)).toLocaleString()}</td></tr>`).join("")
    const html = `<html><head><title>${esc(po.poNumber)}</title><style>body{font-family:system-ui;padding:40px;max-width:800px;margin:0 auto}table{width:100%;border-collapse:collapse}th{text-align:left;padding:8px;border-bottom:2px solid #333;font-size:13px}@media print{body{padding:20px}}</style></head><body><h2 style="margin:0">MacForce Auto Engineering</h2><p style="color:#666;margin:4px 0 30px">Purchase Order</p><div style="display:flex;justify-content:space-between;margin-bottom:24px"><div><strong>${esc(po.poNumber)}</strong><br>${today}</div><div style="text-align:right"><strong>${esc(supplier)}</strong>${phone ? "<br>" + esc(phone) : ""}${email ? "<br>" + esc(email) : ""}${address ? "<br>" + esc(address) : ""}</div></div><table><thead><tr><th>#</th><th>Item</th><th style="text-align:center">Qty</th><th style="text-align:right">Unit Price</th><th style="text-align:right">Amount</th></tr></thead><tbody>${itemRows}<tr><td colspan="4" style="padding:12px 8px;text-align:right;font-weight:700;font-size:16px">Total</td><td style="padding:12px 8px;text-align:right;font-weight:700;font-size:16px">Rs.${total.toLocaleString()}</td></tr></tbody></table>${notes ? "<p style='margin-top:24px;color:#666'><strong>Notes:</strong> " + esc(notes) + "</p>" : ""}<div style="margin-top:60px;display:flex;justify-content:space-between"><div style="text-align:center;border-top:1px solid #999;padding-top:8px;width:200px">Authorized Signature</div><div style="text-align:center;border-top:1px solid #999;padding-top:8px;width:200px">Supplier Signature</div></div></body></html>`
    const win = window.open("", "_blank")
    if (!win) { tt("⚠️ Pop-ups blocked — allow pop-ups for this site and try again"); return }
...
function NewPOForm({ onSave, onCancel, suppliers, supplierRegistry, tt, nextPONum, onSaveQuiet }) {
  ...
  const printPO = () => {
    saveSilent()
    const itemRows = items.map((i, idx) => `<tr><td style="padding:8px;border-bottom:1px solid #eee">${idx + 1}</td><td style="padding:8px;border-bottom:1px solid #eee">${esc(i.name)}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:center">${i.qty} ${esc(i.unit)}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">Rs.${i.unitPrice.toLocaleString()}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right;font-weight:700">Rs.${(i.qty * i.unitPrice).toLocaleString()}</td></tr>`).join("")
    const html = `<html><head><title>${esc(poNum)}</title><style>body{font-family:system-ui;padding:40px;max-width:800px;margin:0 auto}table{width:100%;border-collapse:collapse}th{text-align:left;padding:8px;border-bottom:2px solid #333;font-size:13px}@media print{body{padding:20px}}</style></head><body><h2 style="margin:0">MacForce Auto Engineering</h2><p style="color:#666;margin:4px 0 30px">Purchase Order</p><div style="display:flex;justify-content:space-between;margin-bottom:24px"><div><strong>${esc(poNum)}</strong><br>${today}</div><div style="text-align:right"><strong>${esc(supplier)}</strong>${phone ? "<br>" + esc(phone) : ""}${email ? "<br>" + esc(email) : ""}${address ? "<br>" + esc(address) : ""}</div></div><table><thead><tr><th>#</th><th>Item</th><th style="text-align:center">Qty</th><th style="text-align:right">Unit Price</th><th style="text-align:right">Amount</th></tr></thead><tbody>${itemRows}<tr><td colspan="4" style="padding:12px 8px;text-align:right;font-weight:700;font-size:16px">Total</td><td style="padding:12px 8px;text-align:right;font-weight:700;font-size:16px">Rs.${total.toLocaleString()}</td></tr></tbody></table>${notes ? "<p style='margin-top:24px;color:#666'><strong>Notes:</strong> " + esc(notes) + "</p>" : ""}<div style="margin-top:60px;display:flex;justify-content:space-between"><div style="text-align:center;border-top:1px solid #999;padding-top:8px;width:200px">Authorized Signature</div><div style="text-align:center;border-top:1px solid #999;padding-top:8px;width:200px">Supplier Signature</div></div></body></html>`
    const win = window.open("", "_blank")
    if (!win) { tt("⚠️ Pop-ups blocked — allow pop-ups for this site and try again"); return }
    win.document.write(html)
    win.document.close()
